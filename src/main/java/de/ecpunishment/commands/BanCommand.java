...
public class BanCommand implements CommandExecutor {
    ...
    public BanCommand(EcPunishment plugin) {
        this.plugin = plugin;
    }
    
    @Override
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if (!sender.hasPermission("ecpunishment.ban")) {
            sender.sendMessage(plugin.getMessagesConfig().getMessage("general.no-permission"));
            return true;
        }
        
        if (args.length < 1) {
            sender.sendMessage(plugin.getMessagesConfig().getMessage("ban.usage"));
            return true;
        }
        ...
        String reason = plugin.getReasonsConfig().getDefaultReason("ban");
        ...
        sender.sendMessage(plugin.getMessagesConfig().getMessage("ban.success")
